...
check("G-IMP-1", "all expected questionnaire items are present (15 Core, no body-type Extended item)",
...
check("G-IMP-4", "matching.js deterministic self-test PASS (score semantics unchanged) + height min/max range validation & boundary behavior green (in-range match, out-of-range miss, strict-flag exclusion)",
      ...
      and 'MULTI = ["living", "work"' in MATCH and 'relationshipStyle' in MATCH and "PETS_ACCEPT" in MATCH and "qvals(item)" in MATCH
...
# G-QM-1 — the questionnaire.json master file parses and holds all 15 items.
...
check("G-QM-1", "all 15 required items present with unique ids (exact set)",
...
_e_fields_ok = bool(_eitems) and all(
    ...
    for it in _eitems)
check("G-QM-1", "embedded questionnaire-data block in index.html is a VALID JSON questionnaire (parses, required fields present, 15 items)",
...
GATES = ("G-IMP-1", "G-IMP-2", "G-IMP-3", "G-IMP-4",
         ...
         "constraints")
print("=" * 70)
print("PARTNER-002 UI prototype gates — G-IMP-1..4 (slice 7e) + G-QM-1..5 (slice 9) + G-LG-1..6 (slice 8) + G-WCAG-1..4 (slice 8b) + G-MP-1..5 (slice 11) + G-LA-1..5 (slice 11b) + G-FL-1..5 (slice 13) + G-LOC-1..2 (slice 13-location) + kids-split (slice 14, folded into G-IMP-3/G-QM-4)")
print("=" * 70)
all_pass = True
for gate in GATES:
